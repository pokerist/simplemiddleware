...
exports.HikCentralSigner = void 0;
const crypto_1 = __importDefault(require("crypto"));
class HikCentralSigner {
    static calculateContentMD5(body) {
        if (!body || (typeof body === 'object' && Object.keys(body).length === 0)) {
            return '';
        }
        const content = typeof body === 'string' ? body : JSON.stringify(body);
        return crypto_1.default.createHash('md5').update(content).digest('base64');
    }
    static sign(options) {
        const { method, accept, contentType, path, appKey, appSecret, nonce, timestamp, body } = options;
        ...
        // REQUEST_URI
        const headersToSign = `x-ca-key:${appKey}\n` +
            `x-ca-nonce:${nonce}\n` +
...
exports.HikCentralSigner = HikCentralSigner;
