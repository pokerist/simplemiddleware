...
import { hikClient } from '../hikcentral/client';
import { ExecuteRequest } from '../hikcentral/types';
import { v4 as uuidv4 } from 'uuid';

export default async function executeRoutes(fastify: FastifyInstance) {
  fastify.post('/api/execute', async (request: FastifyRequest<{ Body: ExecuteRequest }>, reply: FastifyReply) => {
    ...
    try {
      const response = await hikClient.execute(endpoint, method, payload || {});
